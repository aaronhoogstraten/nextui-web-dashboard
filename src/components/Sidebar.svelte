...
<aside
	class="bg-sidebar text-text h-screen flex flex-col shrink-0 border-r border-border shadow-[2px_0_12px_rgba(0,0,0,0.3)] z-10 transition-[width] duration-150 overflow-hidden {collapsed
		? 'w-14'
		: 'w-60'}"
>
	<!-- Header -->
	<div
		class="p-4 border-b border-border flex items-center {collapsed
	...
	{#if !collapsed}
		<div class="p-4 border-b border-border">
			...
			{#if isConnected()}
				...
				{/if}
				<ActionButton onclick={disconnect} disabled={isBusy()} variant="danger" class="w-full">
					Disconnect
				</ActionButton>
			{:else}
	...
	<div class="py-2 flex {collapsed ? 'px-2 justify-center' : 'px-5'}">
		<button
			...
			disabled={!isConnected()}
			class="text-text-muted hover:text-text rounded disabled:opacity-30 disabled:cursor-not-allowed {collapsed
				? 'p-1 text-2xl leading-none'
				: 'text-sm'}"
	...
	<!-- External links -->
	<div
		class="p-3 border-t border-border flex items-center {collapsed
			? 'flex-col gap-2 justify-center'
			: 'gap-3'}"
	>
		<a
		...
		>
			<img
				src="{base}/{isDark()
					? 'nextui_vectorized_shadow.svg'
					: 'nextui_vectorized_shadow_dark.svg'}"
				alt="NextUI"
				class="w-6 h-6"
			/>
		</a>
		...
		{#if !collapsed}
			...
			{:else}
				<span class="ml-auto text-xs text-text-muted opacity-60"
					>{APP_VERSION === 'dev' ? 'dev' : `v${APP_VERSION.replace(/^v/, '')}`}</span
				>
			{/if}
		{/if}
	</div>
